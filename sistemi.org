...
* SIMULAZIONE
** ES find
Comando per elencare i primi 30 file *di testo* più grandi del filesystem.
*** SOLUZIONE (sbagliata)
find /home -regex '.*\.txt' -printf "%s %p\n" 2>/dev/null | sort -nrk 1 | head -n 30
bisogna usare -exec{file} ecc.
* INFO
...
Si basa sulla precisione delle release, putando sulla stabilità.
** Debian Structure
*** Architettura
specifica di come funziona un processore(x86 64 bit, arm, ecc...)
*** Debian Constitution
Specifica i processi decisionali, emula una repubblica parlamentare.
Nessuno ha pieno potere su tutto.
**** Project Leader
di tecnico fa poco, è il leader.
Principalmente fa pubblicità sul progetto.
Nomina il segretario di progetto.
**** Segretario di progetto
Gestisce le finanze del progetto e verbalizza gli esiti delle votazioni.
**** Release Team
Assembla i pacchetti che compongono la distro che si scarica da internet.

E altro.
I candidati si autopropongono per entrare nel progetto.
Vengono sottoposti a un intervista tecnica.
Se la superano diventano *mantainer*.
Un manteiner può diventare un *developer*, che può ad 
esempio reclutare altri manteinier.
*** DAM
controlla che l'arruolamento sia corretto. (anche eticamente).

*** Versioni
Ad ogni istante temporale debian offre 3 varianti
+ SO stabile: con software vecchio, ideale da mettere sui server. Non va molto bene per web development ad es.;
+ SO instabile: non molto testate, ma aggiornate. Significa che funziona ma non è stata
  testata in tutti i sottocasi. Con i report della comunity poi gli errori vengono messi a posto.
+ Testing: via di mezzo tra le due precedenti.
*** Repo
Mirror: server web COPIA del server originale messo in diverse *nazioni*
**** Repository
***** dist
Contiene le distribuzioni(passate, attuali, future).
****** stable
main , non-free, contrib. Dipende se voglio anche usare
software non free.
release --> contiene degli hash su file che garantiscono sicurezza.
Controllo sull'hash dei singoli file e sulla signature.
una signature è essenzialmente un hash di un file.
***** doc
contiene documentazione per fare i dischetti
***** indices
Contiene degli indici per fare le ricerche sui pacchetti.
***** non-US
directory obsoleta.
Tempo fa gli algoritmi di criptaggio non potevano essere esportati perchè 
risultavano come armi, quindi serviva un mirror non americano.
***** pool
Contiene i pacchetti veri e propri.
Sono indicizzati per lettera, in modo da rendere più veloce la ricerca.
I pacchetti sono in formato .deb
***** project
****** Experimental 
Gli sviluppatori inseriscono dei pacchetti software che non danno nessuna garanzia.
Son sicuro che parte, ma per il resto non da garanzie.
Repository di debugging dei manteiner.
*** Caricamento di un pacchetto
1) Subito una serie di script verifica che il pacchetto
   sia "integro" e che segua le linee guida di debian.
   Controlli serrati.
2) Il pacchetto entra in unstable
3) Il pacchetto entra in testing
4) Periodo di freeze
**** Passaggio da unstable a testing
Deve restare in unstable per qualche giorno in modo che gli utenti possano reportare
dei bug. Circa 10 giorni. Se il pacchetto non sopravvive viene rimosso da unstable.
Se sopravvive si fanno altri controlli.
+ Il pacchetto non deve avere bug critici (impedisce il funzionamento di un software)
  Se il numero di bug critici è maggiore del numero di bug della versione attualmente
  in testing allora il tutto viene buttato.
+ Il pacchetto deve essere installabile su *tutte* le architetture.
  Esiste un infrastuttura che permette agli sviluppatori di testare sulle varie architetture.
+ Se le dependecies non sono già in testing allora non si può caricare il pacchetto.
+ Le versioni dei pacchetti devono poter convivere.

** SOftware di gestione
*** dpkg
*importante* --> non pensa alle dependencies
*** apt
*importante* --> gestisce anche le dipendenze fra pacchetti.
la risoluzione delle dipendenze è una VISITA TOPOLOGICA di un grafico.

*** Repository
URI --> schema di naming, nel caso dei siti web l'URI coincide con l'URL
deb-src --> per le regole del free software bisogna mettere a
disposizione anche il *pacchetto sorgente*,
+ Sorgente pronto per essere ricompilato
+ Una directory *debian*

Se al posto di usare la "stable" uso la "jessie", rimango fermo a
quella versione, mentre se uso la stable posso aggiornare la distro
e rimanere al passo con gli aggiornamenti.
**** Aggiornamenti non di sicurezza
Avanzamento naturale OPPURE modifica della versione attuale.
**** Stable
Se al posto di bulls-eye metto stable, quando aggiorno mi aggiorna
alla nuova versione stable. Mentre se lascio bullseye, semplicemente
mi aggiorna bullseye.

**** apt update
L'update non tocca i pacchetti, semplicemente sincronizza i pacchetti
localmente. Eì l'*upgrade* che va ad aggiornare i pacchetti.
**** Domanda
Richiedere deb-src
** Installazione e rimozione dei pacchetti
*** apt
Ogni volta che installo un pacchetto, decifrando i metadati
dei file, debian verifica che i pacchetti siano "safe".
Se per caso ci mancano le chiavi pubbliche degli sviluppatori
debian se ne accorge perchè non può verificare l'integrità.
Esiste quindi un pacchetto che fornisce le chiavi degli
sviluppatori
**** Installazione
Ricorsivamente vengono risolte le dipendenze.
+ preconfigurare = prepara l'ambiente all'installazione
+ spacchettamento = spacchetta il pacchetto debian e il pacchetto dei file
+ configurazione = configura i pacchetti per il S.O attuale.
+ post-configurazione = si esegue con uno script
**** KeyRing
pacchetto che offre le chiavi degli sviluppatori
**** rimozione
posso rimuovere un pacchetto mantenendo la configurazione
con apt remove, mentre rimuovendo con purge, rimuovo anche
i file di configurazione.
Se rimuovo un software che ha delle dipendenze, i pacchetti
installati prima per risolvere le dipendenze devono essere 
rimosse usando *autoremove*, che automaticamente rimuove
questo tipo di pacchetti.

Anche con *purge* non possono essere eliminate delle directory 
che servono ad altri pacchetti. Se la cartella serve solo a 
quel pacchetto allora ok.

Il sistema operativo effettua CACHING, ossia non rimuove i pacchetti,
perchè se tanto poi lo riinstallo non devo riscaricarlo.
con apt clean posso rimuovere i pacchetti inutilizzati.
**** Aggiornamento dei pacchetti.
Per aggiornare i pacchetti --> apt update per sincronizzare
                               apt upgrade per aggiornare i paccheti.
Quando sono nella stessa versione di debian basta aggiornare 
i pacchetti per "aggiornare" la distro, perchè le dipendenze
non cambiano.
Se invece aggiorno la versione di debian le dipendenze potrebbero
cambiare.
**** Aggiornamento della distro
se aggiorno la distro --> viene rimosso il vecchio pacchetto
con le vecche dipendenze, installo le nuove dipendenze, aggiorno
il pacchetto.

** Ricerca e analisi dei pacchetti
Se installo un pacchetto e il binario non ha il nome
del pacchetto come faccio ad eseguire il software?
*** Nomi dei pacchetti
lib -dev -> fornisce strumenti per la compilazione
lib senza dev --> fornsice strumenti per l'esecuzione

*** Tipi di simboli (UNIX)
+ Tabella dei simboli statici
+ Tabella dei simboli dinamici
+ Tabella dei simboli debug

*** dipendenze
se la dipendenza serve per installare il pacchetto, allora
non va bene l'installazione orizzontale.
Quindi le pre-dipendenze vengono installate prima.

Depends invece serve per l'esecuzione.
In questo caso l'installazione avviene orizzontalmente.

+ Pre-depends --> verticale
+ Depends --> orizzontale
**** Conflitto
due pacchetti vanno in conf se condividono o scrivono modifiche
sullo stesso file.
**** Raccomanda
Suggerimento dell'installatore.
In realtà nelle ultime distro queste dipendenze "non necessarie"
vengono installate comunque.
**** Sostituisce
Debian capisce che se anche bash-completion e il nuovo software
lavorano sulle stesse directory allora non deve essere considerata
come conflitto/errore.
**** Pacchetti virtuali
Ad esempio il pacchetto Editor di debian.
*** dipendenze inverse
Appunti sulle slide
*** Ricerca pacchetto --> file
Per capire quale pacchetto fornisce un file
#+BEGIN_SRC 
apt-file search -x '^/usr/bin/top$'
#+END_SRC
se mettessi solo top, prenderei anche dei file che
contengono la parola top, ma non sono eseguibili per esempio.
O comunque file che non centrano con top.
*** Unificazione dei binari
apt sa solo che esiste /bin/dd non /usr/bin/dd , 
che è semplicemente una copia di /bin/dd.
Questo a causa dell'unificazione degli eseguibili in /usr/bin.
Per non rompere gli script sono rimasti
anche i vecchi binari.Questa unificazione viene fatta da uno 
script in post-installazione.
*** Metapacchetti
Sono essenzialmente pacchetti con molte dependencies, che
se installati le installano di conseguenza.
*** Pacchetti virtuali
Sono essenzialmente liste di pacchetti.
Se installati con sudo apt install <nome> non succede nulla
*** update alternatives
comando che permette di gestire i link simbolici che puntano
alle applicazioni di default
Per *elencare i PACCHETTI VIRTUALI* si usa il seguente comando
#+BEGIN_SRC 
sudo apt update-alternatives --get-selections
#+END_SRC
**** DOMANDA:
Perchè se uso il comando
#+BEGIN_SRC bash
update-alternatives --list editor
#+END_SRC
mi viene mostrato /usr/bin/emacs invece di /bin/emacs-gtk?
In teoria update-alternatives (come nel caso di nano)
non dovrebbe vedere gli eseguibili nella cartella /bin
prima?
